...
import { BookService } from "@/services/DatabaseServices";
...
export default function Bank() {
    ...
    const handleSaveButton = async (newRow: GridRowModel) => {
        ...
        if (typeof newRow.id === 'number') {
            res = await BookService.put(newRow)
        } else {
            delete newRow.id
            res = await BookService.post(newRow)
        }
    ...
    const handleDeleteButton = (id: GridRowId) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                response = await BookService.delete({ id })
                if (response == 200) {
                    Swal.fire("Saved!", "", "success");
                    fetchData()
                    setTimeout(() => {
