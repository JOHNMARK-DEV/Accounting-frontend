...
import { FinaltaxtaxService } from "@/services/DatabaseServices";
...
export default function Bank() {
    ...
    const handleSaveButton = async (newRow: GridRowModel) => {
        ...
        if (typeof newRow.id === 'number') {
            res = await FinaltaxtaxService.put(newRow)
        } else {
            delete newRow.id
            res = await FinaltaxtaxService.post(newRow)
        }
    ...
    const handleDeleteButton = (id: GridRowId) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                response = await FinaltaxtaxService.delete({ id })
                if (response == 200) {
                    Swal.fire("Saved!", "", "success");
                    fetchData()
                    setTimeout(() => {
    ...
    const fetchData = async () => {
        try {
            let res = await FinaltaxtaxService.getAll()
            if (res.status == 200) {
