...
import { DepartmentService } from "@/services/DatabaseServices";
...
export default function Bank() {
    ...
    const handleSaveButton = async (newRow: GridRowModel) => {
        ...
        if (typeof newRow.id === 'number') {
            res = await DepartmentService.put(newRow)
        } else {
            delete newRow.id
            res = await DepartmentService.post(newRow)
        }
    ...
    const handleDeleteButton = (id: GridRowId) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                response = await DepartmentService.delete({ id })
                if (response == 200) {
                    Swal.fire("Saved!", "", "success");
                    fetchData()
                    setTimeout(() => {
