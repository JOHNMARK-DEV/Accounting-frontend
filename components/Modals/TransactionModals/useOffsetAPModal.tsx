...
interface Iform { 
    onSubmitToParent: (data:BusinessPartnersData | any) => Promise<void> | undefined;
    isOpenModal?: boolean
    onCancel?:  () => void
}

const BusinessPartnersModal = ({onSubmitToParent,isOpenModal = false,onCancel}: Iform) => {

    
    const [formData, setformData] = useState<BusinessPartnersData>()
            ...
            >
                ...
                <Box paddingX={2}>
                    <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        />
                        <TextField label="Code" fullWidth className="mt-4" {...register("code", { value: formData?.code })}></TextField>
                        <TextField label="Name" fullWidth className="mt-4" {...register("name", { value: formData?.name })} />
                        <TextField label="Address" fullWidth className="mt-4" multiline  {...register("address", { value: formData?.address })}
                            maxRows={4}></TextField>
                        {/*  
                        ...
                        <DialogActions className="d-flex justify-center">
                            ...
                            <Button onClick={onCancel} variant="contained" style={{ backgroundColor: 'red' }} >
                                Cancel
                            </Button>
                        </DialogActions>
                    </form>
